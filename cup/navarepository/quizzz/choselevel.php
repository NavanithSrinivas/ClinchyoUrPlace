...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
    ...
    <body>
       ...
       <!-- <div id="preloader" >
		...
		<center><font size="6"><br>Challenge your self with any one category??<font size="5"><br><br><b>
		<form name="level" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>" >
		<select name="level">
...
$dbhandle = mysql_connect($hostname, $username, $password)
  ...
  $sel=$_SESSION['chosen'];
  $sql = "SHOW TABLES FROM `$sel`";
  
$result = mysql_query($sql)or die("no");
while ( $row = mysql_fetch_array($result)) {
   ?><option value="<?php echo $row[0];?>"> <?php echo($row[0] . "<BR>");?> </option><?php
}
 ?>
 </select >
...
$_SESSION['wrong']=0;
$_SESSION['tid']=0;
$_SESSION['num']=mysql_num_rows("select * from `l1`");

?><script>window.location="http://localhost/quiz/test.php" </script><?php } ?>
